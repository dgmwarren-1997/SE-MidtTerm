								...
								$sql = " UPDATE `invoice_record` SET `total`='$totalcost',`amount_rendered`='$txtamount',`amount_change`='$txtchange' WHERE invoice_no  = '$invoice'  ";
						           ...
						           if ($result) {
						           	 $update = "UPDATE `trans_record` SET 
												`transaction_type`='checkout' ,`stat_checkout`='completed',`status`='completed' , `datecompleted`= now() WHERE tempuserid = '$userid' and invoice_no = '$invoice' ";
		 ...
		 <script src="https://kit.fontawesome.com/129b086bc9.js" crossorigin="anonymous"></script>
			 ...
			 <div class="	" style="text-align: center; justify-content: center; margin-top: 200px">
			 	...
			 	<br>
			 	   ...
			 	   <a href="receiptnouser.php?btnview&invoice=<?php echo $invoice?>&txtamount=<?php echo $txtamount?>&txtchange=<?php echo $txtchange?>&totalcost=<?php echo $totalcost?>&userid=<?php echo $userid?>&admin=<?php echo $admin?>" target="_blank" type="submit" name="btnview" class="btn btn-success"  style="width: 200px">View Receipt <i class="fas fa-eye"></i></a>	                  
